...
import { isGeneraliste, isSpecialiste } from '../../types/medecin';
...
export const MedecinList: React.FC = () => {
  ...
  const filterMedecins = async () => {
    ...
    if (filterType === 'generalistes') {
      filtered = filtered.filter(m => isGeneraliste(m));
    } else if (filterType === 'specialistes') {
      filtered = filtered.filter(m => isSpecialiste(m));
    }
  ...
  const columns = [
    ...
    },
    {
      ...
      label: 'Genre',
      render: (medecin: Medecin) => (
        <span className={`px-2 py-1 text-xs rounded-full ${getGenreColor(medecin.genre)}`}>
          {getGenreLabel(medecin.genre)}
        </span>
      ),
    },
    {
      ...
      render: (medecin: Medecin) => (
        <span className={`px-2 py-1 text-xs rounded-full ${
          isSpecialiste(medecin)
            ? 'bg-green-100 text-green-800' 
            : 'bg-gray-100 text-gray-800'
        }`}>
          {isSpecialiste(medecin) ? medecin.specialisation : 'Généraliste'}
        </span>
      ),
    },
    {
  ...
  const generalistes = medecins.filter(m => isGeneraliste(m));
  const specialistes = medecins.filter(m => isSpecialiste(m));
