...
const UserDashboard: React.FC = () => {
  ...
  const { consultations, isLoading: consultationsLoading, error: consultationsError } = useConsultations();
  const { prescriptions, isLoading: prescriptionsLoading, error: prescriptionsError } = usePrescriptions();
  ...
  const recentActivities = useMemo(() => {
    ...
    prescriptions?.slice(0, 2).forEach((prescription) => {
      if (prescription.consultation && isValidDate(prescription.consultation.dateConsultation)) {
        activities.push({
          id: `prescription-${prescription.id}`,
          type: 'prescription',
          title: 'Prescription médicale',
          description: `Type: ${prescription.type}`,
          date: prescription.consultation.dateConsultation,
        });
      }
    });
    ...
    remboursements?.slice(0, 2).forEach((remboursement) => {
      if (isValidDate(remboursement.dateCreation)) {
        activities.push({
          id: `remboursement-${remboursement.id}`,
          type: 'remboursement',
          title: 'Remboursement',
          description: `Montant: ${remboursement.montant}€`,
          date: remboursement.dateCreation,
